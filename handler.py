"""
RunPod Serverless Handler for ComfyUI Wan2.2 Animate (Motion Forge)

This handler is already baked into the runpod/worker-comfyui base image.
This file exists in the repo so RunPod's GitHub scanner can find the
required `runpod.serverless.start()` call.

The actual handler logic lives in the base image at /handler.py.
See: https://github.com/runpod-workers/worker-comfyui
"""
...
# Try to import network_volume diagnostics (available in base image)
try:
